...
class RootServer extends DNSServer {
  ...
  initOptions(options) {
	...
	this.logger.debug("options.noSigZero",options.noSigZero);
    if (options.noSigZero) {
		this.noSigZero = options.noSigZero;
	}
  ...
  sign(msg, host, port) {
	this.logger.debug("this.noSigZero",this.noSigZero);
	if (this.noSigZero) return msg;
